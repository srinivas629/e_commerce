...
const Product = () => {
  ...
  useEffect(() => {
    // when product list changes make sure productData is updated
    feachProductData();
  }, [products, productId]);

  useEffect(() => {
    if (productData && productData.image && productData.image.length) {
      setImage(productData.image[0]);
    }
  }, [productData]);

  return productData ? (
    <div className="border-t-2 pt-10 trastion-opacity ease-in duration-500 opacity-100">
      {/* ----------------------------------Product Data--------------------------------------------- */}
            ...
            {productData.image &&
              productData.image.map((item, index) => (
                <img
                  ...
                  src={item}
                  key={index}
                  className="w-[25%] sm:w-full sm:mb-3 flex-shrink-0 cursor-pointer sm:mr-2"
                  alt=""
                />
              ))}
          </div>
      ...
      <div className="mt-20">
        <div className="flex">
          <b className="border px-5 py-3 text-sm text-gray-600">Description</b>
